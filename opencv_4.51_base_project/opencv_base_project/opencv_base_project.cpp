...
void OCR::detect_rec(Mat& frame)
{
	...
	tickMeter.start();
	//this->detector.forward(outs, this->detector.getUnconnectedOutLayersNames());   ////§ß¨j§Ý§Ý¨[§â©¦§º©È§î
	...
	for (size_t i = 0; i < indices.size(); ++i)
	{
		...
		if (!this->modelRecognition.empty())
		{
			...
			putText(frame, wordRecognized, vertices[1], FONT_HERSHEY_SIMPLEX, 0.6, Scalar(255, 0, 0));
		}
